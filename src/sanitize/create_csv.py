...
class CsvLabelling:
    ...
    def get_files_from(self, input_dir):
        self.politics_files = [(join(input_dir, f)) for f in listdir(input_dir) if (isfile(join(input_dir, f)) and f.endswith('politics.json') )]
        self.conservative_files = [(join(input_dir, f)) for f in listdir(input_dir) if (isfile(join(input_dir, f)) and f.endswith('conservative.json') )]
